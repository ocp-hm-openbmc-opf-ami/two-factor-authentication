...
class TwoFactorAuthImp : IfcBase
{
  public:
    ...
    /** @Enable and Disable 2FA with respect to the received user name and
     * channel number.
     ...
     */
    std::string enableTwoFactorAuthentication(
        std::string userName, uint8_t channelNumber, bool twoFacStatus) override
    {
        std::string url;
        std::string URLVal;
        ...
        if (twoFacStatus)
        {
            retVal = create_secretURL(userName, channelNumber, URLVal,
                                      scratch_code, &scratch_num);
            if (retVal < 0)
            {
                return URLVal;
            }
        ...
        setDbusProperty(userService, userObj, userAttriIntf,
                        "TwoFacEnableStatus", variant);
        return URLVal;
    }
    ...
    bool verifyOTP(std::string userName, std::string otpString) override
    {
        ...
        {
            std::cout
                << "pam_start returned failure..please check config files and "
        ...
        {
            std::cout << "pam_set_item failed while using converstation"
                      << std::endl;
            goto OUT;
        }
        ...
        if (pam_status != PAM_SUCCESS)
        {
            std::cout << "pam_set_item failed while setting the service"
                      << std::endl;
            goto OUT;
        }
  ...
  private:
    /** @brief Trigger PAM 2FA PAM module and Validate received OTP against
     * user.
    ...
    const char* urlEncode(const char* s)
    {
        ...
        if (size > 10000)
        {
            ...
            std::cerr << "Error: Generated URL would be unreasonably large "
                      << std::endl;
            exit(1);
        }
     ...
     */
    int create_secretURL(std::string username, int channel_num,
                         std::string& url, int* scratch_code, int* scratch_num)
    {
        ...
        char secret[(SECRET_BITS + BITS_PER_BASE32_CHAR - 1) /
                        BITS_PER_BASE32_CHAR +
        ...
        std::string specialChar = "@", specialChar1 = "&",
                    issuerVal = "issuer=";
        ...
        secret_fn = SECRET_PATH + channel + (const char*)username.c_str() +
                    SECRET;
        ...
        s_url = getURL(secret, label.c_str(), &encoderURL, use_totp,
                       issuer.c_str());
        ...
        new_scratch_code:;
            ...
            {
                if (read(fd,
                         buf + (SECRET_BITS / 8 + BYTES_PER_SCRATCHCODE * i),
        ...
        secret_fd =
            open((const char*)secret_fn.c_str(),
...
int main(int argc, char** argv)
{
    try
    {
        ...
        auto manager =
            std::make_unique<TwoFactorAuthImp>(bus, TwoFactorAuthRoot);
